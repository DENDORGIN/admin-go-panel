...
func CreateProperty(c *Property) (*PropertyGet, error) {
...
func GetPropertyByItemId(itemId uuid.UUID) (*PropertyGet, error) {
	...
	err := postgres.DB.Where("item_id =?", itemId).First(&property).Error
	if err != nil {
...
func DeleteProperty(id uuid.UUID) error {
	err := repository.DeleteByID(postgres.DB, id, &Property{})
	if errors.Is(err, gorm.ErrRecordNotFound) {
		return nil
	}
	return err
