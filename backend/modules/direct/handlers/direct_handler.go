...
func EditDirectMessage(ctx *gin.Context) {
	messageID, err := uuid.Parse(ctx.Param("messageId"))
	if err != nil {
		ctx.JSON(http.StatusBadRequest, gin.H{"error": "invalid message ID"})
		return
	...
	message, err := repository.EditMessageByID(db, messageID, userID, &body)
	if err != nil {
		if err.Error() == "access denied: user is not the author" {
			ctx.JSON(http.StatusUnauthorized, gin.H{"error": "you are not the author of this message"})
			return
		}
		ctx.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
...
func AddEmojiToMessage(ctx *gin.Context) {
	messageID, err := uuid.Parse(ctx.Param("messageId"))
	if err != nil {
		ctx.JSON(http.StatusBadRequest, gin.H{"error": "invalid message ID"})
		return
	...
	var body models.Reaction
	if err := ctx.ShouldBindJSON(&body); err != nil {
		ctx.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}

	message, err := repository.AddEmojiToMessage(db, messageID, &body)
	if err != nil {
		ctx.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
	}
...
func DeleteDirectMessage(ctx *gin.Context) {
	messageID, err := uuid.Parse(ctx.Param("messageId"))
	if err != nil {
		ctx.JSON(http.StatusBadRequest, gin.H{"error": "invalid message ID"})
		return
	...
	err = repository.DeleteMessageByID(db, messageID, userID)
	if err != nil {
		if err.Error() == "access denied: user is not the author" {
			ctx.JSON(http.StatusUnauthorized, gin.H{"error": "you are not the author of this message"})
			return
